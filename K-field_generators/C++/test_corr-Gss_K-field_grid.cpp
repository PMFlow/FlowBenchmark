// estimate correlation functions and correlation lengths of normal lnK fields with Gaussian correlation
...
#include <stdlib.h>
#include <string.h>
#include "calc_K_2018-Gss-lambda.c"
...
void tcorr_lnK()
{
  lambu=0.0; lambv=0.0;
  for (l=0; l<=L; l++) {
    coru[l]=exp(-(l*dx/l_cor[0])*(l*dx/l_cor[0]));
    corv[l]=exp(-(l*dx/l_cor[1])*(l*dy/l_cor[1]));
    lambu+=coru[l]*dx;
    lambv+=corv[l]*dy;
...
{
  lambu=0.0; lambv=0.0;  
  pfcu=fopen("corrG/glku","w");
  pfcv=fopen("corrG/glkv","w");
  pftcu=fopen("corrG/tglku","w");
  pftcv=fopen("corrG/tglkv","w");
