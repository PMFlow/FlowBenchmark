/* generates lnK and K-fields at given (x,y) with exponential correlation by using the COS-randomization method */
...
/* amplitudes and modes for anisotropic  Exponential correlation: */
void calc_modE()
{
static int imod,idim;
static double g1,g2,dummfac,sq2Pi,norm,gauss_factor;
 ...
 for (imod=0; imod<NMOD; imod++) {
   ...
   norm=1./sqrt(wavenum[imod][0]*wavenum[imod][0]+wavenum[imod][1]*wavenum[imod][1]);
...
/* void calc_K() */
{
  ...
  if (n_realiz > john) {calc_modE(); john = n_realiz;}
  ...
  /*    { */
  /*     if (mee > me) { */
  /* 	for (nt=0; nt<=mee-1; nt++) calc_modE(); */
  /*     } */
  /*     for (nt=0; nt<=me; nt++) calc_modE(); */
  /*     john = 1; */
  /*   } */
